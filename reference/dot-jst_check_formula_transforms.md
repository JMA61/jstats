...
Interaction terms (x \* z, x:z) are unaffected: terms() lists their
component variables as plain names, not calls. When the offending call
is a single function applied to one bare variable, the message includes
a runnable make-the-variable example built from the user's own term and
data-frame name.
