...
never from a message builder: the emitters wrap every message at the
real prefix reserve and treat existing breaks as hard, so a builder-side
wrap at a guessed reserve produces two sets of break points (the S287
jencode double wrap). receive_package()'s structural gate enforces this
as a call-site whitelist (S292). A builder that needs an indent for its
own layout may still call .jst_wrap_indent().
