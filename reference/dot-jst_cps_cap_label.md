...
columns, ending in "..." (three ASCII periods; see
.jst_truncate_ellipsis), when longer. Returns the (possibly shortened)
content only; the caller supplies the row label ("jcomplete()",
"jsubset()", "subset =") and places the content in the trailing detail
column. Display width is measured with nchar(type = "width"), matching
the renderer's dw().
