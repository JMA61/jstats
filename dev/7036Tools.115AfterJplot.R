...
#' regression lines with equation/R\u00b2/band annotations.
#'
...
#'
#' @param x A result object from one of the package's analysis functions, OR a
#'   data frame (data-first form; in this case pass as \code{data} rather than
#'   \code{x}).
...
#'   passed to class-specific methods (e.g. \code{focal}, \code{at},
#'   \code{equation}, \code{r2} for \code{jst_lm}); for the data-first form
#'   these are unquoted variable names.
#'
...
#' @export
#' @importFrom rlang .data
jplot.default <- function(data, ..., by = NULL, type = NULL,
  ...
  if (!missing(data)) {
    mc <- match.call()
    data <- tryCatch(force(data), error = function(e) {
      .jst_missing_comma_error(deparse(mc$data), "jplot", e)
    })
  }
