...
jrelabel <- function(data, var, labels = NULL, var.label = NULL) {
  ...
  # jrelabel() never converts. Mirrors jdeclare_missing()'s type guard.
  if (!is.null(labels)) {
...
# joptions("missing.convention.codes") -- a pool blind to the column -- so
...
# jdeclare_missing follow-up line, the convention-codes provenance line,
...
.jst_jrecode_convention_error <- function(parsed_map, parsed_labels,
  ...
  # S267 (S242 F2): the user's own marker style leads the pair, so a sas
  # user is not shown stata first. The signal is quote_tags -- the same
  # as-typed vector the head quotes -- so the pair order can never
  # disagree with the spelling on screen.
  #
  # TWO WRONG SIGNALS were tried first, both silently inert. phr is
  # unusable here: .jst_phrasing_convention() falls back to the setting,
  # and this message fires only under an spss convention, so phr is
  # "stata" in every case the rule was written for. all_tags is unusable
  ...
  switch_txt <- if (by_call) {
    paste0(
      ...
      "\" on this call to \"", own_first[1L], "\" or \"",
      own_first[2L], "\".")
  } else {
    paste0("To switch conventions instead, run one of:\n",
           "  joptions(missing.convention = \"", own_first[1L], "\")\n",
           "  joptions(missing.convention = \"", own_first[2L], "\")")
  }
...
# simplifications: the rewrite is a single jdeclare_missing call (not two
...
# joptions("missing.convention.codes"), which knows nothing about the column.
#
# joutput-level gating:
#   minimal  - three lines: what went wrong, see ?jdeclare_missing, the
#              joptions switch hint.
#   standard - full block with the rewritten jdeclare_missing call and
...
#'
#' Produces the error message used by \code{jdeclare_missing()} when
  ...
  if (length(na_vals) > 0L) {
    ...
    if (length(rebuilt_parts) > 0L) {
      ...
      msg_parts <- c(msg_parts,
        ...
        paste0("  jdeclare_missing(", data_name, ", ", var_name,
               ", codes = ", codes_arg, ")"),
        "",
...
.jst_jdeclare_udm_mixed_error <- function(parsed_codes, data_name, var_name,
  ...
  if (identical(output_level, "minimal")) {
    return(paste0(
      ...
      " missing values and ",
      "SPSS-style numeric codes. Issue these as separate jdeclare_missing() calls."
    ))
  }
  ...
  tag_line <- paste0("  jdeclare_missing(", data_name, ", ", var_name,
  ...
  num_line <- paste0("  jdeclare_missing(", data_name, ", ", var_name,
  ...
  msg_parts <- c(
    ...
    paste0(
      ...
      "missing-value cells (token input) and converting numeric cells to ",
...
#'   \code{jdeclare_missing()}). \code{NA} may be combined with numeric old
...
#' subsequent \code{jdeclare_missing()} call attaches the label and flags
#' \code{-99} as missing. Labeling \code{-99} inside the \code{labels}
#' argument is unnecessary --- \code{jdeclare_missing()} owns that label.
#'
...
jrecode <- function(data, orig.var, map, labels = NULL, convention = NULL) {
  ...
  # than its level values). Mirrors jdeclare_missing()'s type guard.
  # (Session 215)
    ...
    if (is.null(convention) && opt_conv %in% c("spss", "stata", "sas") &&
      ...
      .jst_stop(
        paste0(
          ...
          "missing.convention setting is \"", opt_conv, "\"."), "\n",
  ...
  # at jdeclare_missing() (the SPSS-style second step; an NA=.a target under
  ...
  if (length(d1_rules) > 0) {
    ...
    if (length(d1_targets) > 0) {
      ...
      if (length(flagged) > 0) {
        ...
        # S267: the resolution honors a per-call convention =. Reading the
        # option alone mis-attributed the mint under a per-call override
        # and the pasted remedy silently reverted to the setting.
        d1_conv <- if (!is.null(convention)) convention else
...
#' codes -- a note suggests declaring it with \code{jdeclare_missing()}
...
#'   \code{\link{jdeclare_missing}} for declaring missing-value codes,
...
jencode <- function(data, var, map = NULL, labels = NULL, convention = NULL) {
  ...
  if (is.null(map)) {
    ...
    if (length(plain_targets) > 0) {
      ...
      if (length(flagged) > 0) {
        ...
        # S267: per-call convention honored; see the jrecode twin's note.
        d1_conv <- if (!is.null(convention)) convention else
                     getOption(".jst_options_missing_convention",
                               .jst_options_defaults$missing.convention)
        d1_unset <- !isTRUE(d1_conv %in% c("spss", "stata", "sas"))
  ...
  # --- Messages, in order ---------------------------------------------------
  # S267 (Rule F): one emission with blank-line joins -- per-entry
  # .jst_msg() calls printed consecutive notes glued together.
  if (length(msgs) > 0L) .jst_msg(paste(msgs, collapse = "\n\n"))
...
#' \code{jdeclare_missing()}, or \code{character(0)} when nothing is
...
#' discrete value; \code{jdeclare_missing()} enforces the same rule on the
...
#' (\code{jdeclare_missing(df, X, range = c(-99, -51))}).
#'
...
#'
#' @export
jdeclare_missing <- function(data, ..., codes = NULL, labels = NULL,
                         range = NULL, vars = NULL, convention = NULL,
                         missing.notice = TRUE, modify = FALSE) {
  ...
  .jst_check_flag(missing.notice, "missing.notice")
  ...
  if (!is.null(labels)) {
    ...
    # S249: jdeclare_missing IS the caller that quotes markers back, so it keeps
  ...
  for (ti in seq_len(n_targets)) {
    ...
    res <- tryCatch({
      ...
      # default). jdeclare_missing labels existing Stata-style missings; it does
      ...
      if (has_tagged && !col_has_stata_form) {
        ...
        .jst_stop(
          paste0(
            "'", vn, "' has no lettered markers to label, so ",
      ...
      if (!is.null(range) && !is.null(existing_conv) &&
        ...
        .jst_stop(
          ...
          "  jconvert(", data_name, ", to = \"spss\", vars = \"", vn,
          "\", modify = TRUE)",
          fn = "jdeclare_missing")
      }
      ...
      # clean-column conflict, so jdeclare_missing can never CREATE a mixed
      ...
      if (!is.null(convention) && !is.null(existing_conv) &&
        ...
        # S267: both remedies runnable (Rule L). Removing the argument
        # lets resolution follow the column's own form (level 1); the
        # corrected call is echoed when echo-renderable, prose otherwise.
        conf_echo_ok <- is.numeric(codes) &&
          all(is.na(haven::na_tag(codes))) &&
          (is.null(names(codes)) || !any(nzchar(names(codes)))) &&
          is.null(labels) && is.null(range)
        conf_alt <- if (conf_echo_ok) {
          paste0(
            "To follow the column's form, remove the argument:\n",
            "  jdeclare_missing(", data_name, ", ", vn, ", codes = c(",
            paste(format(codes, trim = TRUE, scientific = FALSE),
                  collapse = ", "),
            ")", if (isTRUE(modify)) ", modify = TRUE" else "", ")\n")
        } else {
          paste0("To follow the column's form, remove the convention ",
                 "argument from the call.\n")
        }
        .jst_stop(
          paste0(
            "'", vn, "' already carries ", other_form,
            " missing values; cannot use convention = \"", convention,
            "\" here."),
          "\n",
          conf_alt,
          "Or convert the column first:\n",
          "  jconvert(", data_name, ", to = \"", convention,
          "\", vars = \"", vn, "\", modify = TRUE)")
      }
      ...
      if (resolved_convention == "spss" && !is.null(existing_conv) &&
          is.na(existing_conv)) {
        # S267: named styles replace "both letter cases" (the letter-case
        # framing described the mechanism, not the user's data), and the
        # remedy is route-aware: when spss arrived per call OVER a
        # stata/sas setting, removing the argument is the one-step fix,
        # echoed as the corrected call when the call is echo-renderable
        # (plain numeric codes, no labels/range).
        mix_setting <- getOption(".jst_options_missing_convention",
                                 .jst_options_defaults$missing.convention)
        mix_head <- paste0(
          "'", vn, "' carries both Stata-style (.a-.z) and SAS-style ",
          "(.A-.Z) missing values, which SPSS convention cannot act on.")
        mix_echo_ok <- is.numeric(codes) &&
          all(is.na(haven::na_tag(codes))) &&
          (is.null(names(codes)) || !any(nzchar(names(codes)))) &&
          is.null(labels) && is.null(range)
        if (identical(convention, "spss") &&
            isTRUE(mix_setting %in% c("stata", "sas")) && mix_echo_ok) {
          .jst_stop(
            mix_head, "\n",
            "This call sets convention = \"spss\", but your ",
            "missing.convention setting is \"", mix_setting, "\". ",
            "To use the setting, remove the argument:\n",
            "  jdeclare_missing(", data_name, ", ", vn, ", codes = c(",
            paste(format(codes, trim = TRUE, scientific = FALSE),
                  collapse = ", "),
            ")", if (isTRUE(modify)) ", modify = TRUE" else "", ")")
        } else if (identical(convention, "spss") &&
                   isTRUE(mix_setting %in% c("stata", "sas"))) {
          .jst_stop(
            mix_head, "\n",
            "This call sets convention = \"spss\", but your ",
            "missing.convention setting is \"", mix_setting, "\". ",
            "To use the setting, remove the convention argument from ",
            "the call.")
        } else {
          .jst_stop(
            mix_head, "\n",
            "Set convention = \"stata\" or convention = \"sas\" on ",
            "this call.")
        }
      }
  ...
  if (length(build_err) > 0L) {
    ...
    lines <- vapply(seq_along(build_err), function(i) {
      m <- sub("^jdeclare_missing\\(\\): ", "", build_err[[i]])
  ...
  # never-chose. Fires each call: a jdeclare_missing call is a deliberate
  ...
  if (length(override_notes) > 0L && isTRUE(missing.notice)) {
...
.jst_jdeclare_udm_spss <- function(col, parsed_codes, var_name,
  ...
  if (any(!is.finite(code_vals))) {
    .jst_stop("codes must be finite numeric values.", fn = "jdeclare_missing")
  }
  if (any(code_vals != floor(code_vals))) {
    .jst_stop("codes must be whole numbers.", fn = "jdeclare_missing")
  }
  if (anyDuplicated(code_vals) > 0L) {
    .jst_stop("codes contains duplicate values.", fn = "jdeclare_missing")
  }
  ...
  if (identical(output_level, "full") && branch == "stata_conversion") {
    ...
    eq_call <- if (isTRUE(modify)) {
      paste0(
        ...
        "  jdeclare_missing(", data_name, ", ", var_name,
...
#'   (\code{preserve.declarations = FALSE}); \code{\link{joptions}} for setting
#'   the default convention and convention codes session-wide.
#'
#' @export
jconvert <- function(data, to = NULL, ..., vars = NULL, missing.notice = TRUE,
                     modify = FALSE) {
  ...
  if (is.null(to)) {
    ...
    if (convention %in% c("spss", "stata", "sas")) {
      ...
      .jst_stop(
        ...
        "  to = \"stata\"\n",
        # S267: descriptors pre-broken so "base R" sits at a line edge;
        # the block is sever-swept clean across widths 40-120 (see the
        # gate-menu builder note).
        ...
        .jst_wrap_indent(paste0(
          ...
          "numeric codes; jstats treats them as missing."),
          indent = 6L), "\n",
  ...
  if (to %in% c("stata", "sas")) {
    ...
    if (length(over_cap_vars) > 0L) {
      ...
      # S267 redraft: same family treatment as the SPSS-cap block above
      # (Rule L two-space remedies, Rule X, unnumbered alternatives).
      msg_lines <- c(
        ...
        sprintf("  jconvert(%s, to = \"%s\", vars = c(...), modify = TRUE)",
                data_name, to),
        "Or reduce each variable to 26 or fewer values first.")
      .jst_stop(paste(msg_lines, collapse = "\n"))
    }
  }
  ...
  if (isTRUE(missing.notice)) {
    ...
    if (genuinely_empty) {
      if (user_specified) {
        .jst_msg("No declared missing values found in: ",
                 paste(target_vars, collapse = ", "), ".")
      } else {
        .jst_msg("No declared missing values found in '", data_name, "'.")
      }
    ...
    if (all_already_in_target && !user_specified) {
      .jst_msg(sprintf(
        "All variables with declared missing values in '%s' are already in %s-form representation.",
        data_name, to))
      return(invisible(data))
    }
    ...
    if (n_converted > 0L) {
      header_verb <- switch(
        to,
        baseR = "Stripped the missing-value declarations from",
      ...
      # translated. Rides inside the missing.notice gate (Q2: an explicit
      # missing.notice = FALSE suppresses this along with the rest of the
      # report).
    ...
    if (n_pattern_a > 0L) {
      ...
      msg_lines <- c(msg_lines,
                     "",
                     "  To declare these values as missing, see jdeclare_missing().",
